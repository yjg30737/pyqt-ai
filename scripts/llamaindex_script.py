...
class GPTLLamaIndexClass(GPTWrapper):
    """
    ...
    def set_query_engine(self, streaming, similarity_top_k=3):
        if self._index is None:
            raise Exception('Index must be initialized first. Call set_directory or set_files first.')
        try:
            self._query_engine = self._index.as_query_engine(streaming=streaming, similarity_top_k=similarity_top_k)
        except Exception as e:
            raise Exception(f'Error in setting query engine: {e}')

    def get_llamaindex_response(self, text):
        if self._query_engine is None:
            raise Exception('Query engine is not initialized. Call set_query_engine first.')
        try:
